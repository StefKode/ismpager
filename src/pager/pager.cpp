...
#ifdef DEBUG
#define dbg_print(S) Serial.println(S); delay(500);
#else
#define dbg_print(S)
#endif
...
void alarm(uint8_t type, uint8_t flr, uint8_t room)
{
  char *alarmstr;
  int freq, i;
...
int check_and_handle_msg()
{
  ...
  if (rf_ask.recv((uint8_t*)&rxblock, &rxlen)) {
    ...
    if (rxblock.elem.alarm != 0) {
      alarm(rxblock.elem.alarm, rxblock.elem.alarm_floor, rxblock.elem.alarm_room);
      ret = 2;
    }
